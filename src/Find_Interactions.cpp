...
//Probe to Distal Interactions
    ...
    //output to file
    for(auto eniter = enClass.Enhancers.begin() ; eniter != enClass.Enhancers.end(); ++eniter){
            
        for (auto it = eniter->second.proximities.junctions.begin(); it != eniter->second.proximities.junctions.end(); ++it){//key:REpos
            
            if(((abs(it->first - eniter->second.start)) > MinimumJunctionDistance)){
            ...
            if(it->second.reportit){
                ...
                outf1 << eniter->first << '\t' << eniter->second.chr  << '\t' << eniter->second.start << '\t' << eniter->second.end << '\t';
                
               
                outf1 << eniter->second.chr << '\t' << it->first << '\t' << it->second.refragend << '\t';
        ...
        for (auto itx = eniter->second.proximities_ctx.begin(); itx != eniter->second.proximities_ctx.end(); ++itx){
            
            for(auto itt = itx->junctions_ctx.begin(); itt != itx->junctions_ctx.end(); ++itt){
                enoughpairs = CheckSupportingPairs(itt->second.paircount, NumberofExperiments);
                if(enoughpairs){
    ...
    outf1.close();
    count = 0;
//Enhancer- enhancer Interactions
	std::string FileName3;
	FileName3.append(BaseFileName);
    FileName3.append(".");
    FileName3.append(whichchr);
	FileName3.append(".Interactions.Enhancer_Enhancer.");
	FileName3.append("txt");
	std::ofstream outf3(FileName3.c_str());

    
	outf3 << "Enhancer_ID_1" << '\t' << "EnhancerChr_1" << '\t' << "EnhancerStart_1" << '\t' << "EnhancerEnd_1" << '\t';
    
    outf3 <<  "Enhancer_ID_2" <<  '\t' << "EnhancerChr_2" << '\t' << "EnhancerStart_2" << '\t' << "EnhancerEnd_2" << '\t';
	...
	for(auto eniter = enClass.Enhancers.begin() ; eniter != enClass.Enhancers.end(); ++eniter){
        ...
        if (count % 2500 == 0)
            fLog << count << " enhancers processed" << std::endl;
        ++count;
		
	for (itff = eniter->second.Inter_feature_ints.begin(); itff != eniter->second.Inter_feature_ints.end(); ++itff){
			f = itff->interacting_feature_id;
            auto eniter2 = enClass.Enhancers.find(f);
            if( (abs(eniter->second.start - eniter2->second.start) >= MinimumJunctionDistance)){
                outf3 <<  eniter->first << '\t' << eniter->second.chr << '\t' << eniter->second.start << '\t' << eniter->second.end << '\t';
                
                outf3 <<  eniter2->first << '\t' << eniter2->second.chr << '\t' << eniter2->second.start << '\t' << eniter2->second.end << '\t';
                
                if(eniter->second.chr == eniter2->second.chr)
                    outf3 << abs(eniter->second.start - eniter2->second.start) << '\t';
                else
                    outf3 << -1 << '\t';
                for (int e = 0; e < NumberofExperiments; ++e){
                    outf3 << itff->signal[e];
