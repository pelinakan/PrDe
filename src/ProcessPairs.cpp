...
void ProcessBAM::ProcessSortedBAMFileForEnhancers(ProbeSet& ProbeClass, ProbeRESitesClass dpnII, ProximityClass proximities, std::string BAMFILENAME, int ExperimentNo, std::string whichchr, std::string DesignName, std::string StatsOption, EnhancerSet& EnClass){
	...
	bool read1_strand, read2_strand, re1f, re1r;
	enpairinfo.resites1 = new int [2];
	enpairinfo.resites2 = new int [2];
	int i=0;
	int checkifprobe;
	...
	for(auto eniter = EnClass.Enhancers.begin(); eniter != EnClass.Enhancers.end(); ++eniter){
		enRefID =ChrNamestoRefID[eniter->second.chr];
		enRegion.LeftRefID = enRefID;
		enRegion.RightRefID = enRefID;
		enRegion.LeftPosition =  eniter->second.start - padding;
		enRegion.RightPosition =  eniter->second.end+ padding;
	
	
		reader.SetRegion(enRegion);
		while(reader.GetNextAlignmentCore(al)){
			...
			if(whichchr!="chrAll"){
				if(al.RefID == chrindex){
					enpairinfo.chr1 = RefIDtoChrNames[al.RefID];
					enpairinfo.chr2 = RefIDtoChrNames[al.MateRefID];
					enpairinfo.enid1 = eniter->first;
					checkifprobe = ProbeClass.FindOverlaps(enpairinfo.chr2, al.MatePosition, (al.MatePosition + ReadLen), DesignName);
				
					if(checkifprobe!=-1){
						continue;
					}
					enpairinfo.enid2=EnClass.FindOverlaps(enpairinfo.chr2, al.MatePosition, (al.MatePosition + ReadLen));
					if(enpairinfo.enid2=="null"){
						re1r = dpnII.GettheREPositions(enpairinfo.chr2, (al.MatePosition), enpairinfo.resites2);
					if ( !re1r){
						enpairinfo.resites2[0] = al.MatePosition;
						enpairinfo.resites2[1] = al.MatePosition;
					}
				}
